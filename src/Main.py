...
# OpenCV
import cv2
from team710.carControl import (PID, getAngle, getBirdView,
                                  isCommingToCrossroad)
from team710.getMaskByCanny import getLaneMaskByCanny
from team710.getMaskByColor import getMergeMask
from team710.signDetect import Cascade_sign
...
act_img = cv2.imread(path + 'team710/images/sign1.png')
Cascade_sign(act_img)
...
def getDirect(image, _side):
    ...
    if (score < 80):
        ...
        if (score < score2):
            ...
            birdView = birdView2
            avg = avg2
            # cv2.putText(image, 'Using mask color', (10, 220),
            #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2,
            #             cv2.LINE_AA)
        # else:
            # cv2.putText(image, 'Using mask Canny', (10, 220),
...
class image_feature:
    def __init__(self):
        ...
        self.camera_angle__publisher = rospy.Publisher(
            "/team710/set_camera_angle", Float32)

        # subscribed Topic
        self.rgb_subscriber = rospy.Subscriber(
            "/team710/camera/rgb/compressed",
            CompressedImage,
            self.rgb_callback,
            queue_size=1,
            buff_size=85000)
    ...
    def rgb_callback(self, ros_data):
        ...
        # print ('kich thuoc ros_data ', sys.getsizeof(ros_data.data))

        #### direct conversion to CV2 ####
        np_arr = np.fromstring(ros_data.data, np.uint8)
        #image_np = cv2.imdecode(np_arr, cv2.CV_LOAD_IMAGE_COLOR)
        image_np = cv2.imdecode(np_arr, cv2.IMREAD_COLOR)  # OpenCV >= 3.0:
        # print ('kich thuoc image_np ', sys.getsizeof(image_np))
        ...
        #     cv2.putText(image_np, 'Saved', (150, 20), cv2.FONT_HERSHEY_SIMPLEX,
        ...
        if (is_counting_frame == True):
            ...
            # cv2.putText(image_np, 'Detecting Sign', (150, 20),
        ...
        if (turn == True):
            ...
            if (counter_turn <= frame_turn):  #re trong n frame
                ...
                # cv2.putText(image_np, 'Turning ' + str(sideTurn), (150, 40),
                #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2,
                #             cv2.LINE_AA)
                CarAngle, score, mask, out, birdView, image = getDirect(
                    image_np, sideTurn)
        ...
        #             'angle= ' + str(math.ceil(CarAngle * 10000) / 10000),
        #             (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1,
        #             cv2.LINE_AA)
        # cv2.putText(image_np, 'speed= ' + str(CarSpeed), (10, 40),
        #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1,
        #             cv2.LINE_AA)
        # cv2.putText(image_np, 'CamAngle= ' + str(cam_angle.data), (10, 60),
        #             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1,
        #             cv2.LINE_AA)
        angle.data = CarAngle
        speed.data = CarSpeed
